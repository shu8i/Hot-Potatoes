...
public class GridPanel extends JPanel {
	...
	public GridPanel(final int GRID_SIZE, final BuildPanel buildPanel) {
		...
		for (int row = 0; row < GRID_SIZE; row++) {
			for (int col = 0; col < GRID_SIZE; col++) {
				...
				GridCell cellPane = new GridCell(GRID_SIZE, buildPanel, row,
						col);
				Border border;
				if (row < 4) {
					if (col < 4) {
	...
	public GridPanel(final Grid grid, final BuildPanel buildPanel) {
		...
		for (int row = 0; row < grid.getSize(); row++) {
			for (int col = 0; col < grid.getSize(); col++) {
				...
				GridCell gridCell = new GridCell(grid.getSize(), buildPanel,
						row, col);
				Border border;
				if (row < 4) {
					if (col < 4) {
	...
	public void softRefresh() {
		...
		for (final Map.Entry<Coordinate, GridCell> entry : this.references
				.entrySet()) {

			if (entry.getValue().getContent() != null
					&& entry.getValue().getContent()
							.equals(BuildPanelSelection.KAREL)) {
			...
			if (!this.grid.getBlock(entry.getKey()).is(POTATO)
					...
					&& entry.getValue().getContent()
							.equals(BuildPanelSelection.POTATO)) {
				entry.getValue().removeContent();
			}
	...
	private class GridCell extends JPanel {
		...
		public GridCell(int gridSize, final BuildPanel buildPanel, int row,
				int col) {
				...
				public void mouseClicked(MouseEvent e) {
					if (content != null) {
						...
						case POTATO:
							...
							content = null;
							GridPanel.this.grid
									.removePotato(GridCell.this.coordinate);
							break;
						case WALL:
							setBackground(null);
							...
							content = null;
							GridPanel.this.grid
									.removeWall(GridCell.this.coordinate);
							break;
						default:
							break;
						}
		...
		public GridCell(int gridSize, int row, int col) {
			this.cellSize = 500 / gridSize;
			this.coordinate = new Coordinate(col + 1, gridSize - row);
			if (grid.getBlock(this.coordinate).is(KAREL))
				this.content = BuildPanelSelection.KAREL;
			else if (grid.getBlock(this.coordinate).is(WALL))
				this.content = BuildPanelSelection.WALL;
			else if (grid.getBlock(this.coordinate).is(POTATO))
				this.content = BuildPanelSelection.POTATO;
			else if (grid.getBlock(this.coordinate).is(HOME))
				this.content = BuildPanelSelection.HOME;
			references.put(this.coordinate, this);
			((FlowLayout) GridCell.this.getLayout()).setVgap(0);
		}
		...
		private void addKarel(boolean EDIT_MODE) {
			...
			try {
				...
				Image scaledImage = image.getScaledInstance(cellSize, cellSize,
						Image.SCALE_SMOOTH);
		...
		private void addPotato(boolean EDIT_MODE) {
			try {
				...
				GridCell.this.potatoWrapper = new JLabel(
						GridCell.this.potatoIcon);
				GridCell.this.potatoWrapper.setPreferredSize(new Dimension(
						cellSize, cellSize));
