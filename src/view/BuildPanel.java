...
public class BuildPanel extends JPanel {
	...
	public BuildPanel(JFrame parent, LoginPanel loginPanel,
		...
		this.saveMenu.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (!BuildPanel.this.EDIT_MODE) {
					new SaveDialog(BuildPanel.this, BuildPanel.this.gridPanel
							.getGrid(), BuildPanel.this.controller);
				} else {
					BuildPanel.this.controller
							.updateGrid(BuildPanel.this.gridPanel.getGrid());
					Controller.saveDatabase();
					BuildPanel.this.hintPanel.updateHint("Saved Successfully.",
		...
		gridSizeTextField.getDocument().addDocumentListener(
				getDocumentListener());
		c.gridx = 16;
		c.gridwidth = 5;
		c.weightx = 5;
	...
	public BuildPanel updateLoadMenu() {
		...
		for (final Map.Entry<String, Grid> entry : Controller.getGrids()
			...
			loadMenuSubItem.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					BuildPanel.this.parent.setTitle("Editing: "
							+ entry.getKey());
					BuildPanel.this.gridSizeTextField.setText(String
							.valueOf(entry.getValue().getSize()));
	...
	public BuildPanel updateDeleteMenu() {
		...
		this.deleteMenu
				.setEnabled(this.deleteMenu.getMenuComponentCount() != 0);
		return this;
	}
	...
	public void updateGrid() {
		try {
			Integer gridSize = Integer
					.parseInt(BuildPanel.this.gridSizeTextField.getText());
			if (gridSize >= 10 && gridSize <= 50) {
				...
				BuildPanel.this.gridPanel = new GridPanel(gridSize,
						BuildPanel.this);
				c = new GridBagConstraints();
				c.weightx = 20;
				c.gridwidth = 20;
				...
				BuildPanel.this.hintPanel.updateHint(
						"Size must be between 10 and 50!", Color.ORANGE);
			}
	...
	public void updateMenu() {
		this.parent.setJMenuBar(new Menu().buildMenu("Menu", loginPanel,
				controller, this, this.newMenu, this.loadMenu, this.deleteMenu,
				this.saveMenu, this.backMenu));
	}
