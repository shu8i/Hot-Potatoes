...
public class Code implements Serializable {
	...
	private enum Mode {
		IF, IF_DECLARATION, FIRST_IF, ELSE, WHILE, WHILE_DECLARATION, FIRST_WHILE, ACTION, END
	}
	...
	public Iterator<CodeBlock> viewIterator() {
		return new Iterator<CodeBlock>() {
			...
			@Override
			public boolean hasNext() {
				if ((current.defaultCondition != null && !visited
						.contains(current.defaultCondition))
						|| (current.trueCondition != null && !visited
								.contains(current.trueCondition))
						|| (current.falseCondition != null && !visited
								.contains(current.falseCondition))) {
					return true;
				}
				...
				if (current.condParent != null
						...
						&& !visited
								.contains(current.condParent.defaultCondition)) {
					return true;
				}

				if (!mode.isEmpty() && mode.peek().equals(Mode.ELSE)
						&& !elseReturned) {
					return true;
				}
			...
			@Override
			public CodeBlock next() // TODO fix
									// "if x then nothing else something"
			{
				current.setCurrent(false);
				if (current.trueCondition != null
						&& !visited.contains(current.trueCondition)) {
					current = current.trueCondition;
					elseReturned = false;
				} else if (current.falseCondition != null
						&& !visited.contains(current.falseCondition)
						&& !elseReturned) {
					elseReturned = true;
					return new CodeBlock("ELSE", new CodeType(
							CodeType.Type.ELSE));
				} else if (current.falseCondition != null
						&& !visited.contains(current.falseCondition)
						&& elseReturned) {
					...
					elseReturned = false;
				} else if (current.defaultCondition != null
						&& !visited.contains(current.defaultCondition)) {
					current = current.defaultCondition;
					elseReturned = false;
				} else if (current.condParent != null) {
					current = current.condParent;
					if ((current.getCodetype().getType()
							.equals(CodeType.Type.IF)
							&& visited.contains(current.trueCondition) && visited
								.contains(current.falseCondition))
							|| (current.getCodetype().getType()
									.equals(CodeType.Type.WHILE) && visited
									.contains(current.trueCondition))) {
						current = current.defaultCondition;
						elseReturned = false;
					} else {
						if (current.falseCondition != null && elseReturned) {
							current = current.falseCondition;
							elseReturned = false;
						} else if (((mode.peek().equals(Mode.ELSE) && current.defaultCondition == null) || (current.falseCondition != null && !visited
								.contains(current.falseCondition)))
								&& !elseReturned) {
							elseReturned = true;
							return new CodeBlock("ELSE", new CodeType(
					...
					if (((mode.peek().equals(Mode.ELSE) && current.defaultCondition == null) || current.falseCondition != null)
							&& !elseReturned) {
						elseReturned = true;
						return new CodeBlock("ELSE", new CodeType(
								CodeType.Type.ELSE));
	...
	private void addActionCodeBlock(CodeBlock codeBlock, Mode mode) {
		...
		case ACTION:
			if (!this.mode.isEmpty()
					&& (this.mode.peek().equals(Mode.IF_DECLARATION) || this.mode
							.peek().equals(Mode.WHILE_DECLARATION))) {
				this.curr.setCondition(codeBlock.getCodetext());
				this.mode
						.push(this.mode.peek().equals(Mode.IF_DECLARATION) ? Mode.FIRST_IF
								: Mode.FIRST_WHILE);
			} else if (!this.mode.isEmpty()
					&& (this.mode.peek().equals(Mode.FIRST_IF) || this.mode
							.peek().equals(Mode.FIRST_WHILE))) {
	...
	private void removeConditionalBlock(CodeBlock block) {
		...
		if (block.defaultCondition.defaultCondition == null) // if last
																// conditional
																// in code
		{
	...
	private void removeActionBlock(CodeBlock block) {
		...
		{
			if (block.defaultCondition == null) // if the only statement in
												// if/while
			{
		...
		} else if (block.parent.falseCondition == block) // if in first else
															// condition
		{
			if (block.defaultCondition == null) // if the only statement in else
												// (remove entire if/else)
			{
	...
	private Code copy(Code oldCode) {
		...
		CodeBlock codeBlock;
		// create new blocks
		for (Map.Entry<Integer, CodeBlock> entry : oldCode.references
				.entrySet()) {
			codeBlock = new CodeBlock(entry.getValue().getCodetext(), entry
					.getValue().getCodetype());
		...
		for (Map.Entry<Integer, CodeBlock> entry : code.references.entrySet()) {
			...
			if (oldCode.references.get(entry.getKey()).defaultCondition != null)
				codeBlock.defaultCondition = code.references
						.get(oldCode.references.get(entry.getKey()).defaultCondition
								.getId());

			if (oldCode.references.get(entry.getKey()).trueCondition != null)
				codeBlock.trueCondition = code.references
						.get(oldCode.references.get(entry.getKey()).trueCondition
								.getId());

			if (oldCode.references.get(entry.getKey()).falseCondition != null)
				codeBlock.falseCondition = code.references
						.get(oldCode.references.get(entry.getKey()).falseCondition
								.getId());

			if (oldCode.references.get(entry.getKey()).parent != null)
				codeBlock.parent = code.references.get(oldCode.references
						.get(entry.getKey()).parent.getId());

			if (oldCode.references.get(entry.getKey()).condParent != null)
				codeBlock.condParent = code.references.get(oldCode.references
						.get(entry.getKey()).condParent.getId());
		}
		...
		while (!oldCode.conditionals.isEmpty())
			reversedConditionals.push(code.references.get(oldCode.conditionals
					.pop().getId()));
		...
		code.head.defaultCondition = code.references
				.get(oldCode.head.defaultCondition.getId());
	...
	public Code undo() {
		...
		if (this.mode.pop().equals(Mode.ELSE))
			return this;
