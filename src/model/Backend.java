...
public class Backend implements Serializable {
	...
	public static Backend readDatabase() {
		try {
			ObjectInputStream objectInputStream = new ObjectInputStream(
					new FileInputStream(Constants.DATABASE));
	...
	public void saveDatabase() {
		try {
			ObjectOutputStream objectOutputStream = new ObjectOutputStream(
					new FileOutputStream(Constants.DATABASE));
			objectOutputStream.writeObject(this);
		} catch (IOException e) {
			// e.printStackTrace();
			System.out.println("Error: couldn't save database <"
					+ Constants.DATABASE + ">");
		}
	}
	...
	public void addGrid(Grid grid) {
		...
		if (this.grids.get(grid.getName()) != null) {
			throw new IllegalArgumentException("\"" + grid.getName()
					+ "\" already exists.");
		}
	...
	public String toString() {
		...
		return stringBuilder.toString().isEmpty() ? "No users Exist"
